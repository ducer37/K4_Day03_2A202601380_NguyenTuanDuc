...
def get_candidate_profile(candidate_id: str) -> str:
    ...
    if not candidate:
        return (
            f"LỖI: Không tìm thấy ứng viên '{candidate_id}' trong dữ liệu tuyển dụng."
        )
...
def score_candidate_fit(candidate_id: str, job_title: str) -> str:
    ...
    matched_nice = [
        skill for skill in nice_to_have if skill.lower() in candidate_skills
    ]

    skill_score = (len(matched_required) / len(required_skills)) * 70
    min_exp = job["min_experience_years"]
    exp_score = (
        20 if min_exp == 0 else min(candidate["experience_years"] / min_exp, 1) * 20
    )
    bonus_score = (
        0 if not nice_to_have else (len(matched_nice) / len(nice_to_have)) * 10
    )
