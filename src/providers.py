...
class MockProvider(BaseLLMProvider):
    ...
    def _baseline_response(self, prompt: str) -> str:
        ...
        if any(token in text for token in ["c001", "c002", "c003", "c004", "c005", "c006", "c007", "c999", "slot", "xếp lịch"]):
            return (
    ...
    def _react_response(self, prompt: str) -> str:
        ...
        if "c999" in text:
            return (
                "Thought: Ngày hoặc mã ứng viên không hợp lệ, không nên tiếp tục đặt lịch.\n"
                "Final Answer: Tôi chưa thể xếp lịch phỏng vấn vì thông tin đầu vào không hợp lệ. "
                "Vui lòng kiểm tra lại mã ứng viên và ngày phỏng vấn theo định dạng YYYY-MM-DD."
            )

        if "c005" in text and "qa engineer" in text:
            if observation_count == 0:
                return "Thought: Cần kiểm tra slot ngày 2026-08-02 trước khi xếp lịch.\nAction: check_interview_slots[\"2026-08-02\"]"
            if observation_count == 1:
                return "Thought: Slot 11:00 khả dụng, có thể xếp lịch phỏng vấn.\nAction: schedule_interview[\"C005\", \"QA Engineer\", \"2026-08-02\", \"11:00\"]"
            return (
                "Thought: Tôi đã có đủ thông tin để trả lời.\n"
                "Final Answer: Đã tạo lịch phỏng vấn cho ứng viên C005 - Đỗ Hoàng Nam "
                "với vị trí QA Engineer vào 2026-08-02 lúc 11:00."
            )

        if "c003" in text and "backend developer" in text:
            if observation_count == 0:
                return "Thought: Cần chấm mức độ phù hợp của C003 với Backend Developer.\nAction: score_candidate_fit[\"C003\", \"Backend Developer\"]"
            return (
                "Thought: Tôi đã có đủ thông tin để trả lời.\n"
                "Final Answer: Ứng viên C003 chưa phù hợp với Backend Developer. "
                "Fit score 10/100, thiếu Python, Django, PostgreSQL, REST API và kinh nghiệm chưa đạt yêu cầu."
            )
